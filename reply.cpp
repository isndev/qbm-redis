...
std::string ReplyParseError::_err_info(const std::string &type, const ReplyValue &reply) {
...
qb::redis::message parse(ParseTag<qb::redis::message>, const ReplyValue &reply) {
    ...
    if (!e1 || !e2) throw ProtoError("Invalid message format");
...
qb::redis::pmessage parse(ParseTag<qb::redis::pmessage>, const ReplyValue &reply) {
    ...
    if (!e1 || !e2 || !e3) throw ProtoError("Invalid pmessage format");
...
qb::redis::subscription parse(ParseTag<qb::redis::subscription>, const ReplyValue &reply) {
    if (!is_array_or_push(reply) || get_pubsub_size(reply) < 3) {
        throw ProtoError("Expect array or push with 3 elements for subscription");
    }
    auto const *e1 = get_pubsub_element(reply, 1);
    auto const *e2 = get_pubsub_element(reply, 2);
    if (!e1 || !e2) throw ProtoError("Invalid subscription format");
...
std::vector<char> parse(ParseTag<std::vector<char>>, const ReplyValue &reply) {
    if (!is_string(reply)) {
...
std::chrono::milliseconds parse(ParseTag<std::chrono::milliseconds>, const ReplyValue &reply) {
    if (!is_integer(reply)) {
...
std::chrono::seconds parse(ParseTag<std::chrono::seconds>, const ReplyValue &reply) {
    if (!is_integer(reply)) {
        throw ReplyParseError("INTEGER", reply);
    }
...
qb::redis::geo_pos parse(ParseTag<qb::redis::geo_pos>, const ReplyValue &reply) {
    if (!is_array(reply) || reply.as_array().size() < 2) {
        throw ReplyParseError("ARRAY with 2 elements", reply);
    }
    
    return qb::redis::geo_pos{
        parse<double>(*reply.as_array()[0]),
        parse<double>(*reply.as_array()[1])
    };
}

qb::redis::stream_id parse(ParseTag<qb::redis::stream_id>, const ReplyValue &reply) {
    if (!is_string(reply)) {
        throw ReplyParseError("STRING", reply);
    }
...
qb::redis::stream_entry parse(ParseTag<qb::redis::stream_entry>, const ReplyValue &reply) {
    if (!is_array(reply) || reply.as_array().size() < 2) {
...
stream_entry_list parse(ParseTag<stream_entry_list>, const ReplyValue &reply) {
    if (!is_array(reply)) {
...
map_stream_entry_list parse(ParseTag<map_stream_entry_list>, const ReplyValue &reply) {
    if (!is_array(reply)) {
        throw ReplyParseError("ARRAY", reply);
    }
...
qb::redis::score parse(ParseTag<qb::redis::score>, const ReplyValue &reply) {
    if (is_double(reply)) {
...
qb::redis::score_member parse(ParseTag<qb::redis::score_member>, const ReplyValue &reply) {
    if (!is_array(reply) || reply.as_array().size() < 2) {
...
std::vector<qb::redis::score_member> parse(ParseTag<std::vector<qb::redis::score_member>>, const ReplyValue &reply) {
    if (!is_array(reply)) {
        throw ReplyParseError("ARRAY", reply);
    }
    ...
    if (!arr.empty() && arr[0] && is_array(*arr[0]) && arr[0]->as_array().size() >= 2) {
        ...
        for (const auto& elem : arr) {
            if (!elem) throw ProtoError("Null element in score-member array");
    ...
    for (size_t i = 0; i < arr.size(); i += 2) {
        result.push_back(qb::redis::score_member{
            parse<double>(*arr[i + 1]),
            parse<std::string>(*arr[i])
        });
    }
    return result;
}

qb::redis::search_result parse(ParseTag<qb::redis::search_result>, const ReplyValue &reply) {
    if (!is_array(reply)) {
        throw ReplyParseError("ARRAY", reply);
    }
...
qb::redis::cluster_node parse(ParseTag<qb::redis::cluster_node>, const ReplyValue &reply) {
    if (!is_string(reply)) {
        throw ReplyParseError("STRING", reply);
    }
    ...
    std::string port_str = (at_pos != std::string::npos) 
        ? token.substr(colon_pos + 1, at_pos - colon_pos - 1)
        : token.substr(colon_pos + 1);
    ...
    if (!(iss >> node.master)) throw ProtoError("Failed to parse master");
    if (!(iss >> node.ping_sent)) throw ProtoError("Failed to parse ping-sent");
    if (!(iss >> node.pong_received)) throw ProtoError("Failed to parse pong-recv");
    if (!(iss >> node.epoch)) throw ProtoError("Failed to parse epoch");
    if (!(iss >> node.link_state)) throw ProtoError("Failed to parse link-state");
...
qb::redis::memory_info parse(ParseTag<qb::redis::memory_info>, const ReplyValue &reply) {
    if (!is_array(reply)) {
        throw ReplyParseError("ARRAY", reply);
    }
...
qb::redis::pipeline_result parse(ParseTag<qb::redis::pipeline_result>, const ReplyValue &reply) {
    if (!is_array(reply)) {
        throw ReplyParseError("ARRAY", reply);
    }
...
qb::redis::json_value parse(ParseTag<qb::redis::json_value>, const ReplyValue &reply) {
...
qb::json parse(ParseTag<qb::json>, const ReplyValue &reply) {
...
parse(ParseTag<std::vector<std::pair<std::string, double>>>, const ReplyValue &reply) {
    ...
    for (size_t i = 0; i + 1 < reply.as_array().size(); i += 2) {
        result.emplace_back(
            parse<std::string>(*reply.as_array()[i]),
            parse<double>(*reply.as_array()[i + 1])
        );
    }
