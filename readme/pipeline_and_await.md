...
writes one RESP request to the outbound pipe, in call order (`redis.h:811-824`). Redis answers in request order, so the
...
is non-empty and returns the client by reference (`redis.h:858-862`). It does not block the thread in the kernel; each
...
The client holds an internal `std::queue<PendingReply>` (`redis.h:636`). Each callback command registers its handler *
*before** sending bytes, so a fast or synchronous delivery can never run ahead of the queued handler (
`redis.h:814-821`). Because Redis preserves request order on a single connection, the head of the queue always matches
the next reply on the wire.

`pending_reply_count()` returns the current queue depth (`redis.h:864-867`) — useful for tests and for confirming a
drain completed:
...
- It uses `listener::current.run(EVRUN_NOWAIT)`, **not** `qb::io::async::run()`. That distinction is deliberate:
  ...
  may still call `await()` on a second client (`redis.h:852-856`).
- On **disconnect**, the queue is failed: every pending handler runs with `ok() == false` and
  `error() == "disconnected"` (`reply.h:1136-1138`). If an opt-in command deadline tripped first, the failure reason is
  `"command timed out"` instead (`redis.h:778-792`). See [error_handling.md](./error_handling.md).
...
`qb::redis::tcp::pipeline` is an alias for `qb::redis::detail::RedisPipeline<qb::io::transport::tcp>` (`redis.h:1441`);
the SSL transport exposes `qb::redis::tcp::ssl::pipeline` under `QB_HAS_SSL` (`redis.h:1452`). It is a thin, optional
wrapper that holds a reference to a `Redis` client and chains the low-level `command<Ret>(callback, name, args...)` (
`redis.h:938-944`). The reply queue and ordering belong to the client; the wrapper only gives the call site a name.
...
- For the typed mixin methods (`set`, `get`, …), reach the client with `pipe.client()` (`redis.h:930-931`); the wrapper
