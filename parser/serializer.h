...
public:
    // Serialize to string
    [[nodiscard]] static std::string serialize(const Value& value) {
        std::string result;
    ...
    static void serialize_to(const Value& value, std::string& out) {
        std::visit([&out](const auto& v) {
            serialize_impl(v, out);
        }, static_cast<const ValueBase&>(value));
    }
    
    // Serialize simple command array
    template<typename... Args>
    [[nodiscard]] static std::string serialize_command(Args&&... args) {
        std::string result;
    ...
    [[nodiscard]] static std::string serialize_command_array(std::span<const std::string> args) {
        std::string result;
    ...
    [[nodiscard]] static std::string serialize_hello(ProtocolVersion version,
...
private:
    ...
    // Null: _\r\n
    static void serialize_impl(const Null&, std::string& out) {
        out.push_back(type_id::NULL_);
        out.append("\r\n");
    }
    
    // Simple String: +...\r\n
    static void serialize_impl(const SimpleString& s, std::string& out) {
    ...
    // Simple Error: -...\r\n
    static void serialize_impl(const SimpleError& e, std::string& out) {
    ...
    // Integer: :\d+\r\n
    static void serialize_impl(const Integer& i, std::string& out) {
    ...
    // Bulk String: $N\r\n...\r\n
    static void serialize_impl(const BulkString& s, std::string& out) {
        append_bulk_string(out, s.value);
    }
    
    // Boolean: #t\r\n or #f\r\n
    static void serialize_impl(const Boolean& b, std::string& out) {
    ...
    // Double: ,...\r\n
    static void serialize_impl(const Double& d, std::string& out) {
        ...
        if (std::isinf(d.value)) {
            ...
            char buf[64];
            auto [ptr, ec] = std::to_chars(buf, buf + sizeof(buf), d.value,
    ...
    // Big Number: (...)\r\n
    static void serialize_impl(const BigNumber& n, std::string& out) {
    ...
    // Bulk Error: !N\r\n...\r\n
    static void serialize_impl(const BulkError& e, std::string& out) {
        ...
        auto [ptr, ec] = std::to_chars(buf, buf + sizeof(buf), 
                                          static_cast<int64_t>(error_data.size()));
        out.append(buf, ptr - buf);
        out.append("\r\n");
    ...
    // Verbatim String: =N\r\n<encoding>:<data>\r\n
    static void serialize_impl(const VerbatimString& v, std::string& out) {
    ...
    // Array: *N\r\n<elements...>
    static void serialize_impl(const Array& a, std::string& out) {
    ...
    // Map: %N\r\n<key1><val1><key2><val2>...
    static void serialize_impl(const Map& m, std::string& out) {
    ...
    static void serialize_impl(const Attribute& a, std::string& out) {
    ...
    // Set: ~N\r\n<elements...>
    static void serialize_impl(const Set& s, std::string& out) {
    ...
    // Push: >N\r\n<elements...>
    static void serialize_impl(const Push& p, std::string& out) {
    ...
    // Helper functions
    static void append_crlf(std::string& out) {
        out.append("\r\n");
    }
    
    static void append_bulk_string(std::string& out, std::string_view str) {
    ...
    static void append_array_header(std::string& out, size_t count) {
    ...
    static void append_map_header(std::string& out, size_t count) {
    ...
    static void append_set_header(std::string& out, size_t count) {
    ...
    static void append_push_header(std::string& out, size_t count) {
    ...
    static void append_attribute_header(std::string& out, size_t count) {
...
public:
    ...
    // Add argument
    CommandBuilder& arg(std::string_view value) {
    ...
    CommandBuilder& arg(int64_t value) {
    ...
    CommandBuilder& arg(double value) {
        char buf[64];
    ...
    CommandBuilder& arg_optional(const std::optional<std::string>& value) {
        if (value) {
    ...
    CommandBuilder& arg_if(bool condition, std::string_view keyword) {
        if (condition) {
    ...
    CommandBuilder& arg_if(bool condition, std::string_view keyword, std::string_view value) {
        if (condition) {
            _parts.emplace_back(keyword);
            _parts.emplace_back(value);
        }
    ...
    [[nodiscard]] std::string build_with_raw(std::span<const char> extra) const {
...
[[nodiscard]] inline std::string serialize_simple_string(std::string_view str) {
    std::string result;
...
[[nodiscard]] inline std::string serialize_error(std::string_view error) {
    std::string result;
...
[[nodiscard]] inline std::string serialize_integer(int64_t value) {
    std::string result;
...
[[nodiscard]] inline std::string serialize_bulk_string(std::string_view str) {
    std::string result;
...
[[nodiscard]] inline std::string serialize_null() {
    return "_\r\n";
}

[[nodiscard]] inline std::string serialize_array_header(size_t count) {
    std::string result;
