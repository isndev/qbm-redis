...
public:
    ...
    explicit InputBuffer(size_t initial_capacity = DEFAULT_CAPACITY)
        : _buffer(initial_capacity), _read_pos(0), _write_pos(0) {}
    ...
    [[nodiscard]] std::span<const char> readable_span() const noexcept {
    ...
    [[nodiscard]] std::span<const char> readable_span_second() const noexcept {
    ...
    // Find CRLF in buffer
    [[nodiscard]] std::optional<size_t> find_crlf() const noexcept {
        size_t sz = size();
    ...
    [[nodiscard]] std::optional<std::string> extract_line() {
        auto crlf_pos = find_crlf();
        if (!crlf_pos) return std::nullopt;
    ...
    // Extract N bytes
    [[nodiscard]] std::optional<std::string> extract_bytes(size_t n) {
...
public:
    ...
    void reset(std::span<const char> data) noexcept {
        _data = data;
        _position = 0;
    }
    ...
    // Get current span
    [[nodiscard]] std::span<const char> current() const noexcept {
    ...
    [[nodiscard]] std::optional<std::string_view> extract_line_view() {
        auto crlf_pos = find_crlf();
        if (!crlf_pos) return std::nullopt;
    ...
    // Skip expected CRLF
    [[nodiscard]] bool skip_crlf() {
        if (_position + 2 > _data.size()) return false;
