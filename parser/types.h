...
public:
    ...
    [[nodiscard]] ParseErrorCode code() const noexcept { return _code; }
    [[nodiscard]] std::string_view message() const noexcept { return _message; }
    
    [[nodiscard]] const char* what() const noexcept {
        switch (_code) {
            case ParseErrorCode::OK: return "OK";
            case ParseErrorCode::INCOMPLETE_DATA: return "Incomplete data";
            case ParseErrorCode::INVALID_TYPE: return "Invalid type";
            case ParseErrorCode::INVALID_LENGTH: return "Invalid length";
            case ParseErrorCode::INVALID_INTEGER: return "Invalid integer";
            case ParseErrorCode::INVALID_DOUBLE: return "Invalid double";
            case ParseErrorCode::INVALID_BOOLEAN: return "Invalid boolean";
            case ParseErrorCode::INVALID_BIG_NUMBER: return "Invalid big number";
            case ParseErrorCode::INVALID_VERBATIM_FORMAT: return "Invalid verbatim format";
            case ParseErrorCode::NESTING_TOO_DEEP: return "Nesting too deep";
            case ParseErrorCode::BUFFER_OVERFLOW: return "Buffer overflow";
            case ParseErrorCode::PROTOCOL_ERROR: return "Protocol error";
        }
...
struct SimpleError {
    ...
    [[nodiscard]] std::string full_message() const {
        ...
        if (message.empty()) return prefix;
...
struct VerbatimString {
    ...
    [[nodiscard]] std::string_view encoding_view() const noexcept {
...
struct Array {
    ...
    Value* operator[](size_t index) { return elements[index].get(); }
    const Value* operator[](size_t index) const { return elements[index].get(); }
...
struct Value : ValueBase {
    ...
    Value(std::nullptr_t) : ValueBase(Null{}) {}
    Value(const char* str) : ValueBase(SimpleString{std::string(str)}) {}
    Value(std::string str) : ValueBase(SimpleString{std::move(str)}) {}
    Value(int64_t i) : ValueBase(Integer{i}) {}
    Value(int i) : ValueBase(Integer{static_cast<int64_t>(i)}) {}
    Value(bool b) : ValueBase(Boolean{b}) {}
    Value(double d) : ValueBase(Double{d}) {}
    ...
    [[nodiscard]] bool is_simple_string() const noexcept {
        return std::holds_alternative<SimpleString>(*this);
    }
    
    [[nodiscard]] bool is_simple_error() const noexcept {
    ...
    [[nodiscard]] bool is_bulk_string() const noexcept {
    ...
    [[nodiscard]] bool is_big_number() const noexcept {
    ...
    // Accessors
    [[nodiscard]] SimpleString& as_simple_string() {
        return std::get<SimpleString>(*this);
    }
    [[nodiscard]] const SimpleString& as_simple_string() const {
        return std::get<SimpleString>(*this);
    }
    
    [[nodiscard]] SimpleError& as_simple_error() {
        return std::get<SimpleError>(*this);
    }
    [[nodiscard]] const SimpleError& as_simple_error() const {
        return std::get<SimpleError>(*this);
    }
    
    [[nodiscard]] BulkError& as_bulk_error() {
        return std::get<BulkError>(*this);
    }
    [[nodiscard]] const BulkError& as_bulk_error() const {
        return std::get<BulkError>(*this);
    }
    ...
    [[nodiscard]] BulkString& as_bulk_string() {
        return std::get<BulkString>(*this);
    }
    [[nodiscard]] const BulkString& as_bulk_string() const {
        return std::get<BulkString>(*this);
    }
    
    [[nodiscard]] VerbatimString& as_verbatim_string() {
        return std::get<VerbatimString>(*this);
    }
    [[nodiscard]] const VerbatimString& as_verbatim_string() const {
        return std::get<VerbatimString>(*this);
    }
    ...
    [[nodiscard]] Boolean& as_boolean() {
        return std::get<Boolean>(*this);
    }
    [[nodiscard]] const Boolean& as_boolean() const {
        return std::get<Boolean>(*this);
    }
    
    [[nodiscard]] Double& as_double() {
    ...
    [[nodiscard]] BigNumber& as_big_number() {
        return std::get<BigNumber>(*this);
    }
    [[nodiscard]] const BigNumber& as_big_number() const {
        return std::get<BigNumber>(*this);
    }
    ...
    [[nodiscard]] Attribute& as_attribute() {
        return std::get<Attribute>(*this);
    }
    [[nodiscard]] const Attribute& as_attribute() const {
        return std::get<Attribute>(*this);
    }
    
    [[nodiscard]] Set& as_set() {
    ...
    [[nodiscard]] Push& as_push() {
        return std::get<Push>(*this);
    }
    [[nodiscard]] const Push& as_push() const {
        return std::get<Push>(*this);
    }
    
    [[nodiscard]] size_t size() const noexcept {
        if (auto* a = std::get_if<Array>(this)) return a->size();
        if (auto* m = std::get_if<Map>(this)) return m->size();
        if (auto* s = std::get_if<Set>(this)) return s->size();
        if (auto* p = std::get_if<Push>(this)) return p->size();
        if (auto* b = std::get_if<BulkString>(this)) return b->size();
        if (auto* s = std::get_if<SimpleString>(this)) return s->value.size();
        return 0;
    }
...
inline bool Map::operator==(const Map& other) const {
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
        if (*entries[i].first != *other.entries[i].first ||
            *entries[i].second != *other.entries[i].second) return false;
    }
    return true;
}

// Set comparison
inline bool Set::operator==(const Set& other) const {
    if (elements.size() != other.elements.size()) return false;
    for (size_t i = 0; i < elements.size(); ++i) {
        if (*elements[i] != *other.elements[i]) return false;
    }
...
// Push comparison
inline bool Push::operator==(const Push& other) const {
    if (elements.size() != other.elements.size()) return false;
    for (size_t i = 0; i < elements.size(); ++i) {
        if (*elements[i] != *other.elements[i]) return false;
    }
...
[[nodiscard]] constexpr bool is_valid_type_prefix(char c) noexcept {
    switch (c) {
...
[[nodiscard]] constexpr bool is_resp3_type(char c) noexcept {
    switch (c) {
        case type_id::NULL_:
        case type_id::BOOLEAN:
        case type_id::DOUBLE:
...
[[nodiscard]] constexpr bool is_aggregate_type(char c) noexcept {
    switch (c) {
        case type_id::ARRAY:
        case type_id::MAP:
        case type_id::ATTRIBUTE:
