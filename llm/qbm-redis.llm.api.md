...
| `status` | `types.h:476` · struct | Simple-string reply wrapper; `operator bool`/`ok()` true iff payload == "OK". |
| `stream_id` | `types.h:307` · struct | `{long long timestamp; long long sequence;}`; `to_string()` → "ts-seq". |
| `stream_entry` / `stream_entry_list` / `map_stream_entry_list` | `types.h:327-334` | `{stream_id id; qb::unordered_map<std::string,std::string> fields;}`; list; per-stream map. |
| `score` / `score_member` | `types.h:337-354` | `score{double value}`; `score_member{double score; std::string member}` (field order: score then member). |
| `geo_pos` / `geo_distance` | `types.h:293, :301` | `geo_pos{double longitude; double latitude}`; `geo_distance{std::string member; double distance}`. |
| `subscription` | `types.h:465` · struct | `{std::optional<std::string> channel; long long num;}`. |
| `message` / `pmessage` | `types.h:454, :462` | `message{pattern; channel; payload; reply_ptr raw}`; `pmessage : message`. |
| `scan<Out=std::vector<std::string>>` | `types.h:534-535` · struct template | `{std::size_t cursor; Out items;}` (cursor 0 = complete). |
| `json_value` | `types.h:412` · struct | Self-describing JSON-like variant (RedisJSON); RESP big numbers kept as strings. |
| `pipeline_result` | `types.h:405` · struct | `{size_t size; size_t error_count; bool all_succeeded;}` — MULTI/EXEC summary; per-command replies NOT stored (use `Reply<pipeline_result>.raw()`). |
| `memory_info` | `types.h:378` · struct | Parsed INFO-memory section (`size_t` fields, 0 fallback). |
| `cluster_node` | `types.h:364` · struct | One CLUSTER NODES line parsed field-by-field. |
| `search_result` | `types.h:357` · struct | FT.SEARCH per-document `{key; fields; values}`. |
| `error` | `types.h:541` · struct | `{std::string what; reply_ptr raw;}`. |
| `LimitOptions` | `types.h:283` · struct | `{long long offset=0; long long count=-1;}` (count -1 = unbounded). A **negative `offset`** on `zrangebylex`/`zrangebyscore` (and the `zrevrange*` variants) suppresses the `LIMIT` clause entirely (no triplet emitted); the default `{0,-1}` still sends `LIMIT 0 -1` (all). |
...
`to_string(...)` overloads (`types.h:550-561`) render `BitOp/UpdateType/Aggregation/GeoUnit/InsertPosition/ListPosition` to Redis keyword strings.

### Interval types (`types.h:65-276`)
