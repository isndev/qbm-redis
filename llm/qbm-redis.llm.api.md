<!-- Verified-against: qbm-redis @ qb 3.1.0. Source of truth: the headers under qbm/redis/src/qbm/redis/. -->
...
`redis.h:292-315` · `class template` : `qb::io::async::tcp::client<...>`. Owns uri, `RetryPolicy`, connection-state flags, TLS verify-peer flag, liveness token. Handles connect, auto-reconnect, protocol switching.
...
`redis.h:220` · `struct`. Exponential-backoff reconnection config. **All time fields are `qb::duration`.**
...
`redis.h:1714` · `struct`. Plaintext-TCP aliases the build always provides:
...
```
`qb::redis::tcp::ssl::{client,pipeline,cb_consumer,co_consumer}` (`redis.h:1726`) — TLS variants, compiled **ONLY** when `QB_HAS_SSL` is defined (OpenSSL found). The whole `ssl` struct is `#ifdef`-gated; on a TCP-only build these names do not exist.

`qb::redis::database<QB_IO_>` (`redis.h:1708`) · `alias template` = `detail::Redis<QB_IO_>`. Default-transport client alias.
`qb::redis::no_check` (`redis.h:1739`) · `inline constexpr auto no_check = [](auto&&){}` — no-op reply callback for fire-and-forget commands.
...
- `qb::redis::detail::RedisCallbackConsumer<QB_IO_>` (`redis.h:1511`) — set `on_message()` / `on_error()` / `on_disconnected()` (each returns `*this`) before subscribing. Ctor: `uri` + optional callbacks.
- `qb::redis::detail::RedisCoroConsumer<QB_IO_>` (`redis.h:1611`) — coroutine consumer; internal `qb::io::async::channel` buffers `DEFAULT_MSG_CAPACITY=8192`; `on_message_dropped()` reports overflow; `message_channel_capacity()` reports capacity.
  - `receive()` (`redis.h:1691`): `auto receive() -> qb::io::async::task<std::optional<qb::redis::message>>` — pull next message; suspends until one arrives, `nullopt` on channel close.
...
`sorted_set_commands.h:39`. Members are `qb::redis::score_member` (`{double score, std::string member}`).
...
### `qb::redis::Reply<T>`
`reply.h:1108-1109` · `struct template`. Typed command result.
...
| `parse<T>` | `reply.h:305-306` (generic dispatcher) / `reply.h:411` (`ParseTag<T>` overloads) · function template | Convert `parser::Value` → `T` via `ParseTag<T>` dispatch; throws `ReplyParseError`/`ProtoError` on mismatch. |
...
| `put_in_pipe` / `to_redis_string` / `redis_count` | `reply.h:1081, :805-893, :704-757` · serialization | Serialize a command into a qb-io pipe; chrono `seconds`/`milliseconds` overloads emit `.count()` (native-unit command boundary). |
...
|---|---|---|
| `reply_ptr` | `types.h:42` · `= std::unique_ptr<parser::Value>` | Move-only owning reply pointer. |
| `status` | `types.h:507` · struct | Simple-string reply wrapper; `operator bool`/`ok()` true iff payload == "OK". |
| `stream_id` | `types.h:338` · struct | `{long long timestamp; long long sequence;}`; `to_string()` → "ts-seq". |
| `stream_entry` / `stream_entry_list` / `map_stream_entry_list` | `types.h:358-365` | `{stream_id id; qb::unordered_map<std::string,std::string> fields;}`; list; per-stream map. |
| `score` / `score_member` | `types.h:368-385` | `score{double value}`; `score_member{double score; std::string member}` (field order: score then member). |
| `geo_pos` / `geo_distance` | `types.h:324, :301` | `geo_pos{double longitude; double latitude}`; `geo_distance{std::string member; double distance}`. |
| `subscription` | `types.h:496` · struct | `{std::optional<std::string> channel; long long num;}`. |
| `message` / `pmessage` | `types.h:485, :462` | `message{pattern; channel; payload; reply_ptr raw}`; `pmessage : message`. |
| `scan<Out=std::vector<std::string>>` | `types.h:565-566` · struct template | `{std::size_t cursor; Out items;}` (cursor 0 = complete). |
| `json_value` | `types.h:443` · struct | Self-describing JSON-like variant (RedisJSON); RESP big numbers kept as strings. |
| `pipeline_result` | `types.h:436` · struct | `{size_t size; size_t error_count; bool all_succeeded;}` — MULTI/EXEC summary; per-command replies NOT stored (use `Reply<pipeline_result>.raw()`). |
| `memory_info` | `types.h:409` · struct | Parsed INFO-memory section (`size_t` fields, 0 fallback). |
| `cluster_node` | `types.h:395` · struct | One CLUSTER NODES line parsed field-by-field. |
| `search_result` | `types.h:388` · struct | FT.SEARCH per-document `{key; fields; values}`. |
| `error` | `types.h:572` · struct | `{std::string what; reply_ptr raw;}`. |
| `LimitOptions` | `types.h:314` · struct | `{long long offset=0; long long count=-1;}` (count -1 = unbounded). A **negative `offset`** on `zrangebylex`/`zrangebyscore` (and the `zrevrange*` variants) suppresses the `LIMIT` clause entirely (no triplet emitted); the default `{0,-1}` still sends `LIMIT 0 -1` (all). |
...
`to_string(...)` overloads (`types.h:581-592`) render `BitOp/UpdateType/Aggregation/GeoUnit/InsertPosition/ListPosition` to Redis keyword strings.

### Interval types (`types.h:84-307`)
...
- `qb::protocol::redis<IO_>` (`redis.h:82-83`) · `class template : qb::io::async::AProtocol<IO_>` — native RESP2/RESP3 protocol attached to a qb-io session (CRTP). RESP3, `max_nesting_depth=64`, `max_bulk_size=512MiB`, `max_array_size=1,000,000`. `message{std::unique_ptr<parser::Value> reply}` (`redis.h:97-99`) — one parsed reply (ownership moved into `on(message)`).
