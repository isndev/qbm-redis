/**
 * @file qbm/redis/commands/transaction_commands.h
 * @brief Redis transaction command mixin (MULTI/EXEC/DISCARD/WATCH/UNWATCH).
 *
 * Provides the CRTP command mixin used by the Redis client to issue
 * transaction-control commands. Each command exposes both a coroutine-awaitable
 * form and a callback-based asynchronous form, and the mixin keeps a client-side
 * hint of whether a MULTI block is currently open.
 *
