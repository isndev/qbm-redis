...
TEST_F(RedisGeoTest, SYNC_GEO_COMMANDS_GEOADD) {
    ...
    EXPECT_EQ(redis.geoadd(key, 15.087269, 37.502669, "Catania", 13.583333, 37.316667,
                           "Agrigento"),
              2);
}
...
TEST_F(RedisGeoTest, SYNC_GEO_COMMANDS_GEORADIUS) {
    ...
    auto results =
        redis.georadius(key, 13.361389, 38.115556, 200, qb::redis::GeoUnit::KM);
    EXPECT_FALSE(results.empty());
    EXPECT_TRUE(std::find(results.begin(), results.end(), "Palermo") != results.end());

    // Test with options
    auto results_with_dist =
        redis.georadius(key, 13.361389, 38.115556, 200, qb::redis::GeoUnit::KM,
                        {"WITHDIST", "WITHCOORD"});
...
TEST_F(RedisGeoTest, SYNC_GEO_COMMANDS_GEORADIUS_OPTIONS) {
    ...
    auto results_with_dist = redis.georadius(key, 13.361389, 38.115556, 200,
                                             qb::redis::GeoUnit::KM, {"WITHDIST"});
    EXPECT_FALSE(results_with_dist.empty());

    // Test with WITHCOORD option
    auto results_with_coord = redis.georadius(key, 13.361389, 38.115556, 200,
                                              qb::redis::GeoUnit::KM, {"WITHCOORD"});
    EXPECT_FALSE(results_with_coord.empty());

    // Test with WITHHASH option
    auto results_with_hash = redis.georadius(key, 13.361389, 38.115556, 200,
                                             qb::redis::GeoUnit::KM, {"WITHHASH"});
    EXPECT_FALSE(results_with_hash.empty());

    // Test with COUNT option
    auto results_with_count = redis.georadius(key, 13.361389, 38.115556, 200,
                                              qb::redis::GeoUnit::KM, {"COUNT", "1"});
    EXPECT_EQ(results_with_count.size(), 1);

    // Test with SORT option
    auto results_with_sort =
        redis.georadius(key, 13.361389, 38.115556, 200, qb::redis::GeoUnit::KM, {"ASC"});
...
TEST_F(RedisGeoTest, SYNC_GEO_COMMANDS_GEORADIUSBYMEMBER_OPTIONS) {
    ...
    // Test with WITHDIST option
    auto results_with_dist = redis.georadiusbymember(
        key, "Palermo", 200, qb::redis::GeoUnit::KM, {"WITHDIST"});
    EXPECT_FALSE(results_with_dist.empty());

    // Test with WITHCOORD option
    auto results_with_coord = redis.georadiusbymember(
        key, "Palermo", 200, qb::redis::GeoUnit::KM, {"WITHCOORD"});
    EXPECT_FALSE(results_with_coord.empty());

    // Test with WITHHASH option
    auto results_with_hash = redis.georadiusbymember(
        key, "Palermo", 200, qb::redis::GeoUnit::KM, {"WITHHASH"});
    EXPECT_FALSE(results_with_hash.empty());

    // Test with COUNT option
    auto results_with_count = redis.georadiusbymember(
        key, "Palermo", 200, qb::redis::GeoUnit::KM, {"COUNT", "1"});
    EXPECT_EQ(results_with_count.size(), 1);

    // Test with SORT option
    auto results_with_sort =
        redis.georadiusbymember(key, "Palermo", 200, qb::redis::GeoUnit::KM, {"ASC"});
    EXPECT_FALSE(results_with_sort.empty());
}
...
TEST_F(RedisGeoTest, SYNC_GEO_COMMANDS_GEOSEARCH_OPTIONS) {
    ...
    // Test with WITHDIST option
    auto results_with_dist =
        redis.geosearch(key, "Palermo", 200, qb::redis::GeoUnit::KM, {"WITHDIST"});
    EXPECT_FALSE(results_with_dist.empty());

    // Test with WITHCOORD option
    auto results_with_coord =
        redis.geosearch(key, "Palermo", 200, qb::redis::GeoUnit::KM, {"WITHCOORD"});
    EXPECT_FALSE(results_with_coord.empty());

    // Test with WITHHASH option
    auto results_with_hash =
        redis.geosearch(key, "Palermo", 200, qb::redis::GeoUnit::KM, {"WITHHASH"});
    EXPECT_FALSE(results_with_hash.empty());

    // Test with COUNT option
    auto results_with_count =
        redis.geosearch(key, "Palermo", 200, qb::redis::GeoUnit::KM, {"COUNT", "1"});
    EXPECT_EQ(results_with_count.size(), 1);

    // Test with SORT option
    auto results_with_sort =
        redis.geosearch(key, "Palermo", 200, qb::redis::GeoUnit::KM, {"ASC"});
    EXPECT_FALSE(results_with_sort.empty());
}
...
TEST_F(RedisGeoTest, SYNC_GEO_COMMANDS_EDGE_CASES) {
    ...
    // Test with empty key
    auto empty_results = redis.georadius("NonExistentKey", 13.361389, 38.115556, 200,
                                         qb::redis::GeoUnit::KM);
...
TEST_F(RedisGeoTest, ASYNC_GEO_COMMANDS_GEOADD) {
    ...
    redis.geoadd([&](auto &&reply) { result = reply.result(); }, key, 13.361389,
                 38.115556, "Palermo");
...
TEST_F(RedisGeoTest, ASYNC_GEO_COMMANDS_GEODIST) {
    ...
    redis.geodist([&](auto &&reply) { distance = reply.result(); }, key, "Palermo",
                  "Catania");
...
TEST_F(RedisGeoTest, ASYNC_GEO_COMMANDS_GEOHASH) {
    ...
    redis.geohash([&](auto &&reply) { hashes = reply.result(); }, key, "Palermo");
...
TEST_F(RedisGeoTest, ASYNC_GEO_COMMANDS_GEOPOS) {
    ...
    redis.geopos([&](auto &&reply) { positions = reply.result(); }, key, "Palermo");
...
TEST_F(RedisGeoTest, ASYNC_GEO_COMMANDS_GEORADIUS) {
    ...
    redis.georadius([&](auto &&reply) { results = reply.result(); }, key, 13.361389,
                    38.115556, 200, qb::redis::GeoUnit::KM);
...
TEST_F(RedisGeoTest, ASYNC_GEO_COMMANDS_GEORADIUSBYMEMBER) {
    ...
    redis.georadiusbymember([&](auto &&reply) { results = reply.result(); }, key,
                            "Palermo", 200, qb::redis::GeoUnit::KM);
...
TEST_F(RedisGeoTest, ASYNC_GEO_COMMANDS_GEOSEARCH) {
    ...
    redis.geosearch([&](auto &&reply) { results = reply.result(); }, key, "Palermo", 200,
                    qb::redis::GeoUnit::KM);
