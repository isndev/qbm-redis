...
TEST_F(RedisTest, ASYNC_BITMAP_COMMANDS_BITOP) {
    ...
    redis.bitop([&](auto &&reply) {
        result_length = reply.result();
    }, "AND", destkey, {key1, key2});
...
TEST_F(RedisTest, ASYNC_BITMAP_COMMANDS_GETBIT_SETBIT) {
    ...
    redis.setbit([&](auto &&reply) {
        setbit_result = reply.result() == 0;
    }, key, 7, true);
    
    redis.getbit([&](auto &&reply) {
        getbit_result = reply.result() == 1;
    }, key, 7);
