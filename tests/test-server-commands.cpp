...
TEST_F(RedisTest, SYNC_SERVER_CLIENT_MANAGEMENT) {
    ...
    ASSERT_TRUE(
        std::find_if(clients.begin(), clients.end(), [](const std::string &client) {
            return client.find("test_client") != std::string::npos;
        }) != clients.end());
    ...
    if (id_pos != std::string::npos) {
        ...
        if (space_pos != std::string::npos) {
            ...
            // This might fail if the client is not blocked, but we're testing the
            // function call
...
TEST_F(RedisTest, DISABLED_SYNC_SERVER_COMMAND_INFORMATION) {
    ...
    try {
        ...
        if (!info.empty()) {
            ...
            // Our implementation maps it to std::vector<std::map<std::string,
            // std::string>> Where each map represents a command's properties
...
TEST_F(RedisTest, DISABLED_SYNC_SERVER_PERSISTENCE) {
    ...
    try {
        ...
        ASSERT_TRUE(bgsave_status || bgsave_status == "Background saving started" ||
