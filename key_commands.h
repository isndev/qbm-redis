...
public:
    ...
    /// @note If key does not exist, `dump` returns `std::optional<std::string>{}` (`std::nullopt`).
    ...
    move(Func &&func, const std::string &key, long long db) {
        return static_cast<Derived &>(*this).template command<bool>(std::forward<Func>(func), "MOVE", key, db);
    }
