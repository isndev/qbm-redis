    ...
    public:
        ...
        start() {
            if (!_started) {
                ...
                _derived.template command<qb::redis::scan<>>(
                    [self](qb::redis::Reply<qb::redis::scan<>> &&reply) {
                        (*self)(std::forward<qb::redis::Reply<qb::redis::scan<>>>(reply));
                    },
        ...
        operator()(qb::redis::Reply<qb::redis::scan<>> &&reply) {
            ...
            if (reply.ok() && reply.result().cursor) {
                ...
                auto self = this->shared_from_this();
                _derived.template command<qb::redis::scan<>>(
                    [self](qb::redis::Reply<qb::redis::scan<>> &&r) {
                        (*self)(std::forward<qb::redis::Reply<qb::redis::scan<>>>(r));
                    },
                    "SCAN", _cursor, "MATCH", _pattern);
            } else {
                try {
...
public:
     ...
     */
    auto expire(const std::string &key, long long timeout) {
        return derived().template make_coro_command<bool>(
            [this, key, timeout](auto&& callback) {
                this->expire(std::move(callback), key, timeout);
            }
        );
    }
     ...
     */
    auto expireat(const std::string &key, long long timestamp) {
        return derived().template make_coro_command<bool>(
            [this, key, timestamp](auto&& callback) {
                this->expireat(std::move(callback), key, timestamp);
            }
        );
    }
    ...
    auto keys(const std::string &pattern = "*") {
        return derived().template make_coro_command<std::vector<std::string>>(
            [this, pattern](auto&& callback) {
                this->keys(std::move(callback), pattern);
            }
        );
    }
     ...
     */
    auto move(const std::string &key, long long destination_db) {
        return derived().template make_coro_command<bool>(
            [this, key, destination_db](auto&& callback) {
                this->move(std::move(callback), key, destination_db);
            }
        );
    }
    ...
    auto persist(const std::string &key) {
        return derived().template make_coro_command<bool>(
            [this, key](auto&& callback) {
                this->persist(std::move(callback), key);
            }
        );
    }
     ...
     */
    auto pexpire(const std::string &key, long long timeout) {
        return derived().template make_coro_command<bool>(
            [this, key, timeout](auto&& callback) {
                this->pexpire(std::move(callback), key, timeout);
            }
        );
    }
     ...
     */
    auto pexpireat(const std::string &key, long long timestamp) {
        return derived().template make_coro_command<bool>(
            [this, key, timestamp](auto&& callback) {
                this->pexpireat(std::move(callback), key, timestamp);
            }
        );
    }
    ...
    auto pttl(const std::string &key) {
        return derived().template make_coro_command<long long>(
            [this, key](auto&& callback) {
                this->pttl(std::move(callback), key);
            }
        );
    }
    ...
    auto randomkey() {
        return derived().template make_coro_command<std::optional<std::string>>(
            [this](auto&& callback) {
                this->randomkey(std::move(callback));
            }
        );
    }
    ...
    auto rename(const std::string &key, const std::string &new_key) {
        return derived().template make_coro_command<status>(
            [this, key, new_key](auto&& callback) {
                this->rename(std::move(callback), key, new_key);
            }
        );
    }
     ...
     */
    auto renamenx(const std::string &key, const std::string &new_key) {
        return derived().template make_coro_command<bool>(
            [this, key, new_key](auto&& callback) {
                this->renamenx(std::move(callback), key, new_key);
            }
        );
    }
    ...
    auto restore(const std::string &key, const std::string &val, long long ttl,
                 bool replace = false) {
        return derived().template make_coro_command<status>(
            [this, key, val, ttl, replace](auto&& callback) {
                this->restore(std::move(callback), key, val, ttl, replace);
            }
        );
    }
     ...
     */
    auto scan(long long cursor, const std::string &pattern = "*", long long count = 10) {
        return derived().template make_coro_command<qb::redis::scan<>>(
            [this, cursor, pattern, count](auto&& callback) {
                this->scan(std::move(callback), cursor, pattern, count);
            }
        );
    }
    ...
    auto ttl(const std::string &key) {
        return derived().template make_coro_command<long long>(
            [this, key](auto&& callback) {
                this->ttl(std::move(callback), key);
            }
        );
    }
    ...
    auto type(const std::string &key) {
        return derived().template make_coro_command<std::string>(
            [this, key](auto&& callback) {
                this->type(std::move(callback), key);
            }
        );
    }
     ...
     */
    auto wait(long long num_slaves, long long timeout) {
        return derived().template make_coro_command<long long>(
            [this, num_slaves, timeout](auto&& callback) {
                this->wait(std::move(callback), num_slaves, timeout);
            }
        );
    }
     ...
     * @see https://redis.io/commands/wait
     */
    auto wait(long long num_slaves,
    ...
    // =============== New Key Commands (TODO_COMMANDS.md) ===============
    ...
    auto copyKey(const std::string &source, const std::string &destination,
                std::optional<long long> db = std::nullopt, bool replace = false) {
        return derived().template make_coro_command<bool>(
            [this, source, destination, db, replace](auto&& callback) {
    ...
    copyKey(Func &&func, const std::string &source, const std::string &destination,
        ...
        return derived().template command<bool>(std::forward<Func>(func), "COPY",
                                                source, destination, opt);
    }
     ...
     */
    auto expiretime(const std::string &key) {
        return derived().template make_coro_command<long long>(
            [this, key](auto&& callback) {
                this->expiretime(std::move(callback), key);
            }
        );
    }
     ...
     */
    auto pexpiretime(const std::string &key) {
        return derived().template make_coro_command<long long>(
            [this, key](auto&& callback) {
                this->pexpiretime(std::move(callback), key);
            }
        );
    }
    ...
    auto migrate(const std::string &host, int port, const std::string &key, long long db,
        ...
        return derived().template make_coro_command<status>(
            [this, host, port, key, db, timeout, copy, replace, auth](auto&& callback) {
                this->migrate(std::move(callback), host, port, key, db, timeout,
                              copy, replace, auth);
            }
        );
    }
    ...
    migrate(Func &&func, const std::string &host, int port, const std::string &key,
            long long db, long long timeout, bool copy = false, bool replace = false,
            std::optional<std::string> auth = std::nullopt) {
        std::vector<std::string> opt;
        if (copy) opt.push_back("COPY");
        if (replace) opt.push_back("REPLACE");
        if (auth) {
            opt.push_back("AUTH");
            opt.push_back(*auth);
        }
        return derived().template command<status>(std::forward<Func>(func), "MIGRATE",
                                                  host, std::to_string(port), key,
                                                  std::to_string(db),
     ...
     */
    auto objectEncoding(const std::string &key) {
        return derived().template make_coro_command<std::optional<std::string>>(
            [this, key](auto&& callback) {
                this->objectEncoding(std::move(callback), key);
            }
        );
    }
    ...
    objectEncoding(Func &&func, const std::string &key) {
        return derived().template command<std::optional<std::string>>(
            std::forward<Func>(func), "OBJECT", "ENCODING", key);
    }
     ...
     * @see https://redis.io/commands/object
     */
    auto objectFreq(const std::string &key) {
        return derived().template make_coro_command<std::optional<long long>>(
            [this, key](auto&& callback) {
                this->objectFreq(std::move(callback), key);
            }
        );
    }
    ...
    objectFreq(Func &&func, const std::string &key) {
        return derived().template command<std::optional<long long>>(
            std::forward<Func>(func), "OBJECT", "FREQ", key);
    }
     ...
     * @see https://redis.io/commands/object
     */
    auto objectIdletime(const std::string &key) {
        return derived().template make_coro_command<std::optional<long long>>(
            [this, key](auto&& callback) {
                this->objectIdletime(std::move(callback), key);
            }
        );
    }
    ...
    objectIdletime(Func &&func, const std::string &key) {
        return derived().template command<std::optional<long long>>(
            std::forward<Func>(func), "OBJECT", "IDLETIME", key);
    }
     ...
     * @see https://redis.io/commands/object
     */
    auto objectRefcount(const std::string &key) {
        return derived().template make_coro_command<std::optional<long long>>(
            [this, key](auto&& callback) {
                this->objectRefcount(std::move(callback), key);
            }
        );
    }
    ...
    objectRefcount(Func &&func, const std::string &key) {
        return derived().template command<std::optional<long long>>(
            std::forward<Func>(func), "OBJECT", "REFCOUNT", key);
    }
     ...
     */
    auto sortKey(const std::string &key,
                 const std::vector<std::string> &options = {}) {
        return derived().template make_coro_command<std::vector<std::string>>(
            [this, key, options](auto&& callback) {
                this->sortKey(std::move(callback), key, options);
            }
        );
    }
    ...
    std::enable_if_t<std::is_invocable_v<Func, Reply<std::vector<std::string>> &&>,
                     Derived &>
    sortKey(Func &&func, const std::string &key,
            const std::vector<std::string> &options = {}) {
        std::vector<std::string> args = {key};
        args.insert(args.end(), options.begin(), options.end());
        return derived().template command<std::vector<std::string>>(
            std::forward<Func>(func), "SORT", args);
    }
    ...
    auto sortKeyStore(const std::string &key, const std::string &destination,
                      const std::vector<std::string> &options = {}) {
        return derived().template make_coro_command<long long>(
            [this, key, destination, options](auto&& callback) {
                this->sortKeyStore(std::move(callback), key, destination, options);
            }
        );
    }
     ...
     */
    auto sortKeyRo(const std::string &key,
                   const std::vector<std::string> &options = {}) {
        return derived().template make_coro_command<std::vector<std::string>>(
            [this, key, options](auto&& callback) {
                this->sortKeyRo(std::move(callback), key, options);
            }
        );
    }
    ...
    std::enable_if_t<std::is_invocable_v<Func, Reply<std::vector<std::string>> &&>,
                     Derived &>
    sortKeyRo(Func &&func, const std::string &key,
              const std::vector<std::string> &options = {}) {
        std::vector<std::string> args = {key};
        args.insert(args.end(), options.begin(), options.end());
        return derived().template command<std::vector<std::string>>(
            std::forward<Func>(func), "SORT_RO", args);
    }
     ...
     */
    auto waitaof(long long num_local, long long num_replicas, long long timeout) {
        return derived().template make_coro_command<long long>(
            [this, num_local, num_replicas, timeout](auto&& callback) {
    ...
    waitaof(Func &&func, long long num_local, long long num_replicas,
            long long timeout) {
        return derived().template command<long long>(std::forward<Func>(func),
                                                     "WAITAOF", num_local, num_replicas,
                                                     timeout);
    }
};
